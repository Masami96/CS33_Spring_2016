	...
	for(i=0; i<MSIZE; i+=32)
	  {
		for(j=0; j<MSIZE; j+=32)
		  {
#pragma omp parallel for collapse(2) private (a,b)
		      for(a=i; a<i+32; a++)
		      {
		          for(b=j; b<j+32; b++)
		          {
...
#pragma omp parallel for private (i,j)
	for(j=1; j<chuck_size; j++) {
	  //#pragma omp parallel for
	  int cat = (chuck[j-1])/(j+1);
	  int meow = j*array_size;
...
void Func5(int b[], int a[])
{
	...
	long log_N=log2(VSIZE);
	     ...
	     stride=(1<<i)-1;
	     step=stride2+1;
	     int pow1 = (int)pow(2, i+1);
	     int pow2 = (int)pow(2, i);
	     for(j=0; j<VSIZE; j+=pow1) {
	         temp=b[j+pow2-1];
	              b[j+pow2-1] = b[j+pow1-1];
	              b[j+pow1-1] = temp+b[j+pow1-1];
	             }
	     }
}
